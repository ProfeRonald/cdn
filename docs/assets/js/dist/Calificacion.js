...
function RevaluaRP(id_nota) {
		...
		var nota = Number($("#" + id_nota).val());
		var permiso = $("#" + id_nota).attr("op");
		var notail = Number($('#ind-'+id_nota.substring(0, id_nota.length - 3)).text());
		...
		if (permiso == 1) {
		  ...
		  if (notail == '' || notail == 0 || notail >= ponderacion_academica) {
			$("#" + id_nota).prop("readonly", true);
			$("#" + id_nota).removeAttr("style");
...
function SumaA(a){
	...
	function RevaluaAsignatura(a) {
		...
		if(pasis < ponderacion_asistencia){
			...
			$('#op1-' + a).closest('tr').hide();
						...
						$('#cpc-' + a).hide();
						$('#cpc-' + a + '-3').text('AUS');
						var cc = Number(parseFloat(cf / 2).toFixed(0));

						}else{
						
						$('#cpc-' + a).show();
						$('#cpc-' + a + '-3').text('');
						var cc = Number(parseFloat(cf / 2).toFixed(0)) + Number(parseFloat($('#cpc-' + a).val() / 2).toFixed(0));
		...
		if($('#e-' + a).val() == 'A'){
			...
			$('#cpex-' + a).hide();
			$('#cpex-' + a + '-3').text('AUS');
			var cex = Number(parseFloat(cf * 0.3).toFixed(0));
		}else{
			$('#cpex-' + a).show();
			$('#cpex-' + a + '-3').text('');
			var cex = Number(parseFloat(cf * 0.3).toFixed(0)) + Number(parseFloat($('#cpex-' + a).val() * 0.7).toFixed(0));
		}
		...
		$('#total-' + a).text(cex);
			...
			var cef = cf;
					...
					$('#op1-' + a).show();
					$('#op1-' + a + '-3').text('');
					var cef = Number(parseFloat(cf + ($('#op1-' + a).val() / 100) * (100 - cf)).toFixed(0));

				}
					
					$('#total-' + a).text(cef);

						if(Number(cef) >= ponderacion_academica){
	...
	function RevaluaModulo(m) {
		...
		$(".ra" + m).each(function (a, c) {
			...
			var ra_4 = Number($('#' + idc[0] + '-' + idc[1] + '-4').val());
			var ras = [ra_1, ra_2, ra_3, ra_4];
			
			var tras = Math.max(...ras);
			var pra = Number($('#pra-' + idc[0] + '-' + idc[1]).val());
			tpra += pra;

				if(tras >= (pra * ponderacion_tecnica)) {
			sras += tras;
			$('#pra-' + idc[0] + '-' + idc[1]).parent().css({'background-color':'#333'});
				}else{
			$('#pra-' + idc[0] + '-' + idc[1]).parent().css({'background-color':'#FF7B5A'});
			ttra++;
				}
		});

		total = Number(parseFloat(sras).toFixed(0));

		if (total < (ponderacion_tecnica * 100)) {
				if(fechaextraordinario == 1){
			$('#a' + m + ' small').html(' - <span class="text-danger">&iexcl;Reprobado!</span>');
				}else{
			$('#a' + m + ' small').html(' - <span class="text-danger">&iexcl;Pendiente!</span>');	
				}
		} else if(tpra == 100 & total > 0 && total >= (ponderacion_tecnica * 100)){
